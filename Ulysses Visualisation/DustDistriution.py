...
for i in range(start_index, end_index):
    condition = ulysses_data[i,indices['rotation_angle_index']] != 999 and ulysses_data[i,indices['quality_flag_index']] >= min_quality_flag
    if condition:
        ulysses_data_without_999.append(ulysses_data[i])

ulysses_data_without_999 = np.array(ulysses_data_without_999)

plt.hist(ulysses_data_without_999[:,indices['velocity_index']])
plt.show()
